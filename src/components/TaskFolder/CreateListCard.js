      ...
      <div className={styles.createListCard}>
        <div>
          <input
            ...
            value={listCardTitle}
            onChange={(e)=>{setListCardTitle(e.target.value)}}
          ></input>
        </div>
        <div className={styles.addListDiv}>
          <button className={styles.addListBtn} onClick={addListCardBtnHandler}>
            Add list
          </button>
          <span>
            <IoIcon4.IoIosClose
              className={styles.addListCancelIcon}
              onClick={()=>{props.addListCardBtnHandler(false,null)}}
            ></IoIcon4.IoIosClose>
          </span>
        </div>
      </div>
